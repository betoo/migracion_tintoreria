...
if(function_exists('xdebug_disable')) { xdebug_disable(); }
	...
	********************************/
	...
	//recorre bd  
	foreach ($db_old as $db => $table) { //nombre db = > array de tabla
		//recorre tablas
		foreach ($table as $table_name => $columns) {  // nombre de tabla => nombre de columnas 
			...
			$new_table['new_table']=(isset($columns['new_table']))?$columns['new_table']:false ;
			$new_table['type_table']=(isset($columns['type_table']))?$columns['type_table']:false ;
			...
			unset($columns['type_table']);//quita del array  
			...
			foreach ($columns as $column_old => $column_new) {
				...
				if (isset($column_new['meta_data_new_table']) ) {//ingresa registros a  nuevas tablas 
					...
					//concatena columnas para insert 
				...
				}else if (isset($column_new['many_to_many']) ) { 

					//agrega relaciones 
					$columns_old.=$column_old.",";
					$columns_new.=$columns[$column_old]['many_to_many']['fk_name'].",";
			...
			if ($new_table['new_table'] != false OR trim($new_table['new_table'])!='' ) {

				//obtiene total de registros					
				$result =$link->query("SELECT count($column_old) as total from $name_db_old.$table_name ");//obtiene total de registros
				...
				for ($i=0; $i < $new_table['num_consultas'] ; $i++) { 
					...
					while ($line = mysqli_fetch_assoc($result)) {
						foreach ($line as $key => $val) { 
							...
							$val=ucfirst(strtolower(trim($val))); //quita espacio y modifica tipo de letra  a capital
							...
							if (is_array($columns[$key])) { //GENERANDO RELACIONES
								...
								}elseif (isset($columns[$key]['many_to_many'])) { // relación  entre tablas 

									$pk_name			=$columns[$key]['many_to_many']['pk_name'];
									$field_name			=$columns[$key]['many_to_many']['field_name'];
									$name_table			=$columns[$key]['many_to_many']['table_name'];

									$pk_name_old			=$columns[$key]['many_to_many']['pk_name_old'];
									$field_name_old			=$columns[$key]['many_to_many']['field_name_old'];
									$table_name_old			=$columns[$key]['many_to_many']['table_name_old'];

									$insert_if_not_exist=$columns[$key]['many_to_many']['insert_if_not_exist'];
									$not_in				=in_array($val,$columns[$key]['many_to_many']['not_in']);

									$resuls =$link->query("SELECT  $pk_name_old FROM $name_db_old.$table_name_old where $field_name_old='$val'");//obtiene registro para latabla nueva (elimna duplicados)			
									
									if ($resuls->num_rows>0) { //encuentra registro y cambia por id 

										$asd=null;
										while ($asd= mysqli_fetch_assoc($resuls)) {
