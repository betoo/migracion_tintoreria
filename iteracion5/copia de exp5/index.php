	...
	********************************/
	...
	foreach ($db_old as $db => $table) { //nombre db = > array la tabla
		...
		foreach ($table as $table_name => $columns) {  // nombre de tabla => nombre de columnas 
			...
			$new_table['is_document']=isset($columns['is_document'])? $columns['is_document'] :false;
			...
			//unset($columns['is_document']);//quita del array  
			...
			foreach ($columns as $column_old => $column_new) {
				...
				}else if(is_array($new_table['is_document'])) {

					//agrega en tabla de documentos, los parametros son : tabla, conexion, db new, db old, nombre de tabla
					add_document($table_name,$link,$name_db_new,$name_db_old,$db_old[$name_db_old][$table_name]);
