	...
	********************************/
	...
	foreach ($db_old as $db => $table) { //nombre db = > array la tabla
		...
		foreach ($table as $table_name => $columns) {  // nombre de tabla => nombre de columnas 
			...
			$new_table['is_document']=$columns['is_document'];
			...
			foreach ($columns as $column_old => $column_new) {
				...
				if (isset($column_new['meta_data_new_table']) and $new_table['new_table'] != false) {//ingresa registros a  nuevas tablas 
					echo "------->";
				...
				}else if($new_table['is_document']=='true' and $new_table['new_table']==false) {

					add_document($table_name,$link,$name_db_new,$name_db_old,$db_old[$name_db_old][$table_name],$new_table['num_consultas']);

					exit();	
				}else if (isset($column_new['relations']) and $new_table['new_table']!=false) { 
